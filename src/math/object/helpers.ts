...
export function _getInnerValue<T extends object, TKey extends keyof DeepReadonly<T>>(o: DeepReadonly<T> | number, key: TKey): DeepReadonly<T>[TKey] | number {
    if (typeof o === 'number') {
        return o;
    }
    if (o == null) {
...
export function doOps<T extends object, TOps extends IObjectOps<any>>(
    ...
    o: DeepReadonly<T>,
    processor?: (ops: TOps, value: DeepReadonly<T[string & keyof T]>, key: string & keyof T) => T[keyof T],
) {
    const res = { } as T;
