import { type IEvent, Event } from './event.js';
...
export class TransitionObserver<T> extends Loggable implements IDisposable {
    ...
    constructor(protected readonly reaction: TransitionReaction<T>, getter?: () => T) {
        super();
        if (getter) {
            this.observe(getter);
        }
    ...
    reverse() {
        ...
        return new TransitionObserver<T>(this.reaction, this._getter)
            .from(this._to!)
            .to(this._from!);
    }
...
export function waitFor<T>(reaction: TransitionReaction<T>, current: () => T, toBe: T) {
